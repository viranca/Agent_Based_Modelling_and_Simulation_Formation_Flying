...
class Flight(Agent):
    ...
    def __init__(
        ...
        self.savings_threshold = 0
        self.auction_value = 0
        self.prev_bid_placed = 0
        self.arrival_time = 0
        self.arrived = False
        self.finished = False
        
        #40% becomes an alliance member, these are randomly divided over the agents
        if self.unique_id < round((self.model.alliance_amount)*self.model.n_flights):
            self.alliance = True
        else: 
            self.alliance = False
        
        #40% becomes a manager and these are randomly divided over the agents
        if self.unique_id < self.model.amount_managers:
            self.manager = 1
            self.contractor = 0
    ...
    def step(self):
        if self.state == "flying":
            if self.model.total_flight_time > 150: #This makes sure the agents only move in the first few steps, the area allocation needs to start if all flights have taken off
    ...
    # =========================================================================
    # This function is meant to calculate the distance from self destination to another point p2.
    # =========================================================================    
    ...
    # =========================================================================
    # This function is the original new joining point method, it is accurate but it took to long to run.
    # Therefore, using the same logic, calculate_new_joining_point2 was created.
    # =========================================================================        
    ...
    # =========================================================================
    # This function computes the new joining and leaving point.
    # =========================================================================    
    def calculate_new_joining_point_2(self, target_agent):
        ...
        new_path1 = 999999
        new_path2 = 999999
        new_path_before1 = new_path1 - 1
        new_path_before2 = new_path1 - 1
        
    # =========================================================================
    # Find optimal theta and compute the joining point
    # =========================================================================    
        while new_path_before1 - new_path1 < 0.1:
            new_path_before1 = new_path1
            ...
            joining_point.append(mp1[0] + m)
            joining_point.append(mp1[1] + n)
            L_3 = self.calc_distance3(target_agent.pos, joining_point)
            new_path1 = L_1 + 2*(b-X)*0.75 + L_3
            
    # =========================================================================
    # Find optimal phi and compute the leaving point
    # =========================================================================        
        while new_path_before2 - new_path2 < 0.1:
            new_path_before2 = new_path2
            ...
            leaving_point.append(mp2[0] + Y_x)
            leaving_point.append(mp2[1] + Y_y) 
            L_4 = self.calc_distance3(leaving_point, target_agent.destination)
    ...
    def do_move(self):
        ...
        #agents go into this loop if they have arrived. The specific savings for the contractors, managers,
        # allicance and non alliance members get calculated, just as the total and individual arrival times
        #_____________________________________________________________________
        if self.arrived == True and self.finished == False: 
            self.arrival_time = self.model.total_flight_time/self.model.n_flights
            self.model.total_arrival_time += self.arrival_time
            self.model.individual_arrival_time.append(self.arrival_time) 
            
            if self.alliance == True:
                self.model.total_alliance_savings += self.fuel_consumption
            else:
                self.model.total_nonalliance_savings += self.fuel_consumption
                
            if self.manager == True:
                self.model.total_manager_savings += self.fuel_consumption
            else:
                self.model.total_contractor_savings += self.fuel_consumption
            self.finished = True
        ...
        if self.state == "flying":
            ...
            if self.model.joining_method == 0: 
                ...
                self.fuel_consumption += f_c
                new_pos = np.round(new_pos.astype(np.double), 2)
            #The lines below were added so the agents would not go off bound, and instead stay on the map
            if new_pos[0] > (self.model.width - 0.01):
                new_pos[0] = (self.model.width - 0.01)
            elif new_pos[0] < 0:
                new_pos[0] = 0
            if new_pos[1] > (self.model.height - 0.01):
                new_pos[1] = (self.model.height - 0.01)
            elif new_pos[1] < 0:
                new_pos[1] = 0
    
            if new_pos[0] > (self.model.width*1.05) or new_pos[1] > (self.model.height*1.05):
                raise Exception("Agents is of the map ")
            self.model.space.move_agent(self, new_pos)


    #This function is used to find a specific list of agents, that are manager of contractor and in an alliance or not
    def find_neighbors(self, role, search_range, formation_state): 
        ...
        if role == "all": 
            for agent in flights_airports: 
                if type(agent) is Flight:
                    neighbors.append(agent)
        if formation_state == "formation":
            for agent in neighbors: 
                if agent.formation == False: 
                    del neighbors[neighbors.index(agent)]
        if formation_state == "not formation":
            for agent in neighbors: 
    ...
    #finds the highest potential fuel savings among a list of agents
