...
def compute_total_arrival_time(model):
    return model.total_arrival_time
...
####added
def flight_arrivals(model):
    return model.individual_arrival_time

def average_alliance_fuel(model):
    return (model.total_alliance_savings/model.amount_alliance_members)

def average_nonalliance_fuel(model):
    return (model.total_nonalliance_savings/(model.n_flights - model.amount_alliance_members))

def average_manager_fuel(model):
    return (model.total_manager_savings/model.amount_managers)

def average_contractor_fuel(model):
    return (model.total_contractor_savings/(model.n_flights - model.amount_managers))