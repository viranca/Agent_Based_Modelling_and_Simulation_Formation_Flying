...
class Flight(Agent):
    ...
    def step(self):
        if self.state == "flying":
            ...
            if self.model.negotiation_method == 1:
                do_CNP(self)
                print(self.unique_id, self.manager, self.auctioneer, give_id_list(self.agents_in_my_formation), len(self.agents_in_my_formation))
                if self.best_target != 0 and self.auctioneer == 0: 
                    print(self.best_target.unique_id)
                #print(model.managers_list)
                print("---------------------------------------------------")
    ...
    def start_formation(self, target_agent, bid_value, discard_received_bids=True):
        ...
        if self.model.negotiation_method == 1:
            ...
            self.auctioneer = 0
            target_agent.auctioneer = 1
            target_agent.manager = 0
            target_agent.accepting_bids = 0
