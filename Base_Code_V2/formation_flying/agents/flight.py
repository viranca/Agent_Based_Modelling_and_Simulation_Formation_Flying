...
class Flight(Agent):
    ...
    def calculate_new_joining_point(self, target_agent):
        ...
        if (self.pos == target_agent.pos).all():
            optimal_theta = 0
        else: 
        ...
        if (self.destination == target_agent.destination).all():
            optimal_phi = 0
        else:     
    ...
    def do_move(self):
        ...
        if self.state == "flying":
            ...
            #new_pos = list(map(lambda x:round(x, 5), new_pos))
    ...
    def calc_speed_to_joining_point(self, neighbor):
        ...
        joining_point = self.calculate_new_joining_point(neighbor)[2]
